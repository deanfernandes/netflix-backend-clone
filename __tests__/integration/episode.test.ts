...
describe("query episode", () => {
  test("episode base fields", async () => {
    ...
    expect(data.episode.id).toBe("1");
    expect(data.episode.title).toBe("Episode 1");
    expect(data.episode.synopsis).toBe("Synopsis 1");
    expect(data.episode.durationMinutes).toBe(45);
    expect(data.episode.number).toBe(1);
  });
  ...
  describe("field resolvers", () => {
    test("season field resolver for episode", async () => {
      ...
      expect(data.episode.season.id).toBe("1");
      expect(data.episode.season.number).toBe(1);
      expect(["U", "PG", "_12", "_15", "_18"]).toContain(
        data.episode.season.ageRating
      );
      expect(data.episode.season.releaseYear).toBe(2023);
    });
  });
});
